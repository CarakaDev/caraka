...
Four files carry it and all four must say the same thing: `src/data/status.ts` in the RELEASE STATE stat, `src/data/compare.ts` in the maturity row, `src/data/security.ts` in what we do not claim, and `src/data/readme.ts` in the status chip. That fourth surface went unnamed here until 14 August 2026 and drifted exactly as far as being unnamed predicts — the chip read `v1.2` while the three named files read `v1.1`, and the line directly below the chip, in the same file, read `v1.5`. The number in all four is the shipped release line and it moves with `package.json`. A `v1.1` that records when something changed — `uninstall` came back in v1.1, the mode key arrived in v1.1 — is history and never moves. The npm chip in `src/data/readme.ts` is the one number that does not track the release: it tracks the registry, so it stays behind until the owner publishes, and its comment says so and must keep saying so.
