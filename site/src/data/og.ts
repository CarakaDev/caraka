...
// v0.1 shipped and is seven phases stale at v1.0. Names and marks follow
// src/data/status.ts; the ink shades are the comp's own. The bar is binary —
// full where the release shipped, empty where the work has not started —
// because the comp's 62% and 48% were a drawing and no document measures a
// fraction of a phase. Every row is kesumba rather than green for the reason
// status.ts gives: each of the eight keeps a field gate open in roadmap.md,
// phase 7's being the launch itself.
//
// `PHASES` in site/scripts/gen-assets.mjs draws the same eight rows into
// og-status.png and is not this file; the two have to move together.
export const phases = [
  { n: '0', glyph: '●', name: 'Spike', tone: '#E2452C', ink: '#F6F9FC', fill: '100%' },
