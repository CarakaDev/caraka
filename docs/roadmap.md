...
- [ ] **Titen:** `titen bootstrap` + `titen serve`; POST `/v1/observations`, `/v1/context/compile`; ukur latensi compile. Sisi kliennya ada: `src/memory/titen.ts` memanggil kedua endpoint itu, dan `caraka doctor` menyelidik `/health` sebelum menyebut memori sehat. Lawan bicaranya yang tidak ada. Pada 8 Agustus 2026 `titen` tidak terpasang di mesin ini dan tidak ada proses yang menjawab di `127.0.0.1:7717`, jadi `bootstrap`, `serve`, dan angka latensi compile tidak punya sumber untuk diukur. Kotak ini terbuka sampai ada Titen hidup yang bisa dijalankan terhadapnya
...
- [ ] Cakupan agent — tujuh preset terkirim di `presets/agents/`. Lima menyatakan blok `acp:` dan dijalankan oleh satu klien ACP yang mengambil perintah spawn-nya dari preset (`claude-code`, `amp`, `cursor`, `gemini`, `goose`); tiga menyatakan `command:` untuk driver CLI generik (`claude-code`, `codex`, `aider`), jadi `claude-code` membawa kedua jalur. Yang pernah dijalankan terhadap biner hidup di mesin ini ada **dua**: **Claude Code** lewat jalur ACP (`scripts/smoke-claude.mjs`) dan **Codex** lewat jalur CLI (`scripts/smoke-cli.mjs`, lulus pertama kali pada 10 Agustus 2026 dengan dua giliran dan satu resume yang mengingat angka giliran pertama). Codex lulus setelah presetnya dibetulkan: `codex exec resume` tidak menerima `--color` maupun `--sandbox`, jadi baris resume selama ini membawa bendera yang ditolak biner dan sandbox yang tidak pernah berlaku — persis jenis kesalahan yang hanya ditemukan dengan menjalankannya. Lima preset sisanya **terjangkau, bukan teruji**: perintah dan bendera mereka datang dari matriks riset, dan kelima berkas menandai sendiri bahwa isinya belum diverifikasi (`aider`, `amp`, `cursor`, `gemini`, `goose`). Baris ini dulu berbunyi "≥ 15 agent tercakup (7 diuji langsung, sisanya via ACP Registry)" — tujuh yang diuji langsung tidak pernah terjadi, dan pembacaan ACP Registry dicabut dari discovery pada 8 Agustus 2026 (`done/driver-v04/plan.md`), sehingga satu-satunya cara cakupan bertambah adalah satu berkas YAML lagi. Sasaran G2 di `prd.md` (≥ 15 agent) belum terpenuhi
