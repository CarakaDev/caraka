...
Warna **tidak** diperbarui di setiap transisi, dan tidak pernah bisa: `icon_color` ditetapkan sekali saat `createForumTopic` dan `editForumTopic` tidak punya field untuk mengubahnya. Kolom warna di atas adalah warna yang sebuah topic akan lahir dengan seandainya state itu yang pertama; yang benar-benar bergerak cuma prefiks namanya. Baris ini menuliskan warna sebagai sesuatu yang diperbarui sampai 15 Agustus 2026, dan tidak ada kode yang pernah melakukannya. **Daftar topic tetap menjadi papan status yang bisa dibaca sekilas** — tanpa membuka satu pun percakapan.
...
Kepemilikan disimpan per thread, bukan per sesi, sehingga sesi kedua yang lahir di topic yang sama lewat `/new` mewarisinya. Nama yang **Caraka sendiri tulis** disimpan sejak 1.5.7, di `topic.name.<chat>.<thread>` sebelah catatan kepemilikan, dan hanya sesudah panggilan rename-nya berhasil. Ia ada untuk satu hal: melewatkan `editForumTopic` yang akan menulis nama yang sudah terpasang, karena tiap rename menulis satu service message `forum_topic_edited` ke transkrip dan nama yang sama dua kali adalah galat 400 yang penelannya tidak bisa membedakannya dari kegagalan izin. Yang tetap tidak bisa Caraka ketahui adalah nama yang **bukan** ia tulis: Bot API tidak punya method yang mengembalikan nama topic, jadi nama yang tertimpa oleh versi sebelum 1.3.1 tetap tidak bisa dipulihkan, dan topic yang diganti namanya dengan tangan membuat catatan ini basi untuk tepat satu transisi. Basis data yang lahir sebelum rilis itu tidak memuat catatan kepemilikan sama sekali, jadi setiap thread di dalamnya terbaca bukan milik Caraka: sesi yang sudah punya topic berhenti memperbarui glifnya, dan topic yang dibuat sesudahnya berjalan seperti biasa.
...
| Pesan di topic sesi **yang sudah `done`** | Lanjutkan sesi yang sama (ACP `session/load`) dan kembalikan penanda state lewat `editForumTopic`. Tidak ada yang dibuka kembali, karena sejak 1.5.2 tidak ada yang menutup sendiri: `done` adalah keadaan yang ditinggalkan satu run, bukan akhir sesi. Cabang `reopenForumTopic` ikut dicabut bersama penutupan otomatisnya, dan baris ini menuliskannya sebagai perilaku hidup sampai 15 Agustus 2026. Yang masih menutup adalah `/close`, dan topic yang ditutup begitu memang tetap tertutup — di grup hanya admin ber-`can_manage_topics` dan pencipta topic yang bisa menulis di sana. |
