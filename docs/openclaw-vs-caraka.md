...
- Verifikasi hidup menyentuh lima dari sembilan preset agent yang dikirim, lewat enam jalur: Claude Code di ACP dan di rute CLI-nya, Codex dan aider di CLI, goose dan opencode lewat ACP. Run itulah yang membetulkannya — dua di antaranya sempat mengirim flag yang ditolak binernya. Empat sisanya menandai `# belum diverifikasi` di berkasnya sendiri: gemini, cursor, dan amp menjawab handshake ACP lalu berhenti di akun berbayar, dan antigravity belum pernah melewati jendela OAuth-nya.
...
- Inti sudah melewati plafonnya sendiri dan terus berjalan: 10.552 baris pada 19 Agustus 2026 terhadap ~8.000 yang ditulis di `AGENTS.md`. v1.0 menyentuh batas itu pada 7.880; v1.1 melewatinya, dan `AGENTS.md` mencatat tiap rilis sesudahnya dengan angka pengukuran dan apa yang dibelinya, bukan dengan plafon yang dinaikkan. Satu pass penyederhanaan mengembalikan 73 baris, dan pass kedua mengukur lima kandidat pelipatan yang dijanjikan empat spec sebagai 35 baris ke arah sebaliknya.
...
Bagi Caraka jalur itu justru seluruh produknya. Lewat ACP, protokol JSON-RPC yang dibuat Zed dan di-co-lead JetBrains dengan 28+ agent di registry-nya, coding agent tetap memegang tool, sandbox, dan konteks repo miliknya sendiri, dan permintaan izinnya (`session/request_permission`) dirender menjadi tombol di chat. Sejak `0.4` jalur itu berhenti berbentuk Claude: sembilan preset dikirim sebagai berkas YAML, driver CLI generik menjalankan agent yang belum bicara ACP, dan pemilihan jatuh dari ACP ke CLI ketika adapternya tidak ada di mesin. Satu test memuat preset dari satu berkas YAML dan membuktikan satu giliran penuh sampai ke chat tanpa `src/core/` disentuh.
...
Soal bobot mesin, angka Caraka adalah sasaran yang dinyatakan di `docs/prd.md`: paket < 15 MB, RAM idle < 80 MB, cold start < 2 detik, inti ≤ 8.000 baris. Butir pengukuran RAM dan cold start di Fase 0 roadmap belum dicentang, jadi tiga angka pertama belum berhak disebut hasil. Yang terakhir sudah terukur dan meleset: 10.552 baris pada 19 Agustus 2026, dengan tiap pelampauan dicatat di `AGENTS.md`.
