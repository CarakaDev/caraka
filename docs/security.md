...
dulu tidak pernah dilihat sekarang dilihat, dan yang terlihat bukan nol: versi
yang dipin, `6.7.18`, berada di bawah advisory critical GHSA-qvv5-jq5g-4cgg, dan
npm menandai versi itu deprecated karena advisory yang sama. Angkanya di §13.
...
| `npm audit` bersih + dependensi dikunci | deferred (8 Agustus 2026) | `package-lock.json` mengunci pohonnya, dan alasan lama sudah tertutup: satu job CI menjalankan `npm audit --omit=dev --audit-level=high` atas pohon produksi, lalu atas pohon yang memasang peer opsional Baileys pada versi yang dipin `package.json`. Yang membuka kotak ini sekarang jawabannya, bukan ketiadaan perintahnya. Pohon produksi menjawab nol; pohon kedua menjawab tiga temuan — satu critical, satu high, satu moderate — karena `@whiskeysockets/baileys@6.7.18` sendiri berada di bawah GHSA-qvv5-jq5g-4cgg (spoofing pesan lewat `protocolMessage` yang dibuat khusus) dan menarik `music-metadata` serta `file-type` yang rentan. npm menandai versi yang dipin itu deprecated dengan advisory yang sama, dan langkah keduanya keluar 1 pada 8 Agustus 2026. Karena kegagalan itu tidak bisa diperbaiki oleh perubahan siapa pun hari ini, langkah kedua ditandai `continue-on-error`: ia tetap melapor dan tidak lagi menjatuhkan `ci` di setiap push, sebab check merah permanen adalah check yang orang belajar abaikan. Tertutup begitu pin naik ke versi tanpa advisory itu, `continue-on-error` dilepas, dan kedua langkah hijau |
