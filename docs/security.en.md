...
| T6b | Disclosure in a group | Stated at pairing, not controlled — §4 item 6. The same holds for a Discord guild channel: the approval card, paths, diffs, and command output are readable by every member who can see that channel | Groups default to `read-only`, enforced on the run path since the mode gate (§5); if it is too sensitive for members to see, a group is not where it belongs |
...
tree. The tree nothing used to look at is looked at now, and what shows up is
not zero: the pinned `6.7.18` is itself under the critical advisory
GHSA-qvv5-jq5g-4cgg, and npm marks that version deprecated over the same
advisory. The figures are in §13.
...
| `npm audit` clean + dependencies locked | deferred (8 August 2026) | `package-lock.json` locks the tree, and the old reason is closed: one CI job runs `npm audit --omit=dev --audit-level=high` over the production tree, then over a tree that installs the optional Baileys peer at the version `package.json` pins. What holds this box open now is the answer, not the missing command. The production tree answers zero; the second tree answers three findings — one critical, one high, one moderate — because `@whiskeysockets/baileys@6.7.18` is itself under GHSA-qvv5-jq5g-4cgg (message spoofing through a crafted `protocolMessage`) and pulls a vulnerable `music-metadata` and `file-type`. npm marks that pinned version deprecated over the same advisory, and the second step exits 1 as of 8 August 2026. Because no contributor's change can fix that failure today, the second step is marked `continue-on-error`: it still reports and no longer sinks `ci` on every push, since a permanently red check is a check people learn to ignore. Closed once the pin moves to a version without that advisory, the `continue-on-error` comes off, and both steps are green |
