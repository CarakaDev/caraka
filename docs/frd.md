...
| ID | P | Kebutuhan |
|---|---|---|
| FR-CHAN-01 | P0 | **Telegram (Bot API 10.2) adalah channel pertama.** Baris ini dulu menulis "satu-satunya channel di v1.0"; sejak v0.5 Discord terpasang sebagai channel kedua lewat interface yang sama (`docs/adr/0008-discord-sebagai-channel-kedua.md`), dan sejak v0.6 WhatsApp sebagai ketiga, dua provider di balik satu `id` (`docs/adr/0009-approval-lewat-kode-di-channel-tanpa-tombol.md`). Signal P2, belum dijadwalkan. |
...
| FR-CHAN-02 | P0 | Semua channel mengimplementasikan interface `Channel` yang sama (`src/core/channel.ts`) dan mendeklarasikan `caps`, dan core melakukan **fallback anggun** bila sebuah kemampuan absen. Terpasang sejak v0.5: `threads`, `buttons`, `maxChars`; `edit` menyusul di v0.6 — empat yang punya pembaca di core. Sejak v0.6 permission tidak lagi ditolak ketika `caps.buttons` bernilai false: kartu membawa satu kode sekali pakai yang dibangkitkan server, dan balasan `ok <kode>` memutuskannya. Kodenya bearer secret yang sama kelasnya dengan callback bertanda tangan — terikat `(principal, sesi, permintaan)`, ber-TTL, sekali pakai — jadi yang berpindah ke chat adalah kodenya, bukan wewenang untuk menyetujui lewat kata biasa. |
...
| FR-CHAN-04 | P0 | Inbound: teks, gambar, dokumen, voice note (path file lokal disediakan ke agent). Voice note ditranskripsi hanya bila user mengonfigurasi transcriber. Separuh pertama terbangun 13 Agustus 2026 dan hanya untuk gambar: sembilan slot media Telegram diklasifikasi menjadi kata netral, caption menjadi teks pesannya, dan gambar diunduh ke `~/.caraka/inbox/<run>/` lalu diserahkan ke agent — sebagai blok konten `image` di rute ACP, sebagai `-i <path>` di rute CLI codex. Yang belum: dokumen, voice note, video, sticker, dan lokasi diklasifikasi, diaudit, dan dijawab satu kalimat yang menyebut jenisnya, karena hanya gambar yang punya pembaca di ketiga rute; unduhan WhatsApp dan Discord juga belum ada, jadi keduanya mengklasifikasi saja. Transkripsi tetap menunggu transcriber yang dikonfigurasi operator, dan tidak ada satu pun di repositori ini. |
| FR-CHAN-05 | P0 | Outbound: teks (dengan code block) dan gambar. Gambar datang sebagai content block ACP — byte base64 beserta mime-nya, bukan path — dan dikirim lewat `Channel.sendImage?`, yang Telegram dan Discord punya dan WhatsApp tidak. Baris ini menuliskan konvensi `MEDIA:<path-atau-url>` sejak spesifikasi dan tidak ada kode yang pernah membangunnya; ia dicabut pada 15 Agustus 2026 karena mengangkat path dari teks agent berarti membaca berkas atas perintah masukan yang model ancaman proyek ini sendiri sebut tidak tepercaya (`docs/security.md` §9). Mengirim berkas sembarang keluar tetap tidak ada. |
...
| FR-CHAN-09 | P1 | Grup: default `requireMention: true`; balasan hanya di thread/reply asal. Separuh pertama terpasang 13 Agustus 2026: adapter melaporkan `InboundMessage.addressed`, dan pesan biasa di container non-privat yang tidak menyapa tidak diserahkan ke jalur agent. Tidak ada opsi `requireMention` untuk dimatikan; yang menentukan adalah jawaban channel, dan channel yang tidak bisa tahu tetap dijawab. Gerbangnya berlaku di dalam topic sesi juga: sejak 1.4.3 klausa yang memperlakukan setiap pesan di topic ber-sesi sebagai ditujukan kepada Caraka dihapus, karena memegang sesi bukan hal yang sama dengan disapa, dan kartu pairing sudah menjanjikan aturan yang lebih ketat kepada ruangan. Jalan masuknya tetap dua dan keduanya satu gerakan: menyebut bot, atau membalas salah satu pesannya — dengan balasan ke service message pembuat topic dikecualikan, karena di topic yang Caraka buka setiap pesan tingkat pertama secara teknis membalasnya. Perintah dan kode approval sengaja lolos gerbang, supaya `/stop` dan `/lock` tetap bekerja di ruang yang tidak menyapa. Separuh kedua belum: balasan mengikuti thread pesan asal, tetapi tidak pernah terikat ke `reply_to_message`, dan perintah global dijawab di General (`docs/session-model.md` §5). |
...
| FR-TOPIC-05 | P0 | Sesi selesai ditandai lewat `editForumTopic` setelah pesan ringkasan penutup dikirim, sehingga baris terakhir selalu menjelaskan hasilnya, lalu topic-nya ditutup lewat `closeForumTopic`. Terpasang 14 Agustus 2026 untuk grup, di mana `can_manage_topics` mencakup close dan reopen dan pencipta topic dikecualikan sama sekali. Topic di DM **tidak** ditutup: kedua method itu didokumentasikan hanya untuk supergroup, jadi panggilannya dijawab galat dan galatnya ditelan. Topic tidak pernah dihapus: method penghapus mengambil seluruh transkrip, dan namanya tidak muncul di `src/`. |
| FR-TOPIC-06 | P0 | Pesan di dalam topic sesi melanjutkan sesi itu dan tidak pernah membuat topic baru — di ruangan, hanya bila pesan itu ditujukan kepada Caraka (FR-CHAN-09); di percakapan pribadi setiap pesan ditujukan kepadanya menurut definisinya. Pesan di topic sesi yang sudah `done` memanggil `session/load`, mengembalikan penanda state lewat `editForumTopic`, dan membuka kembali topic-nya lewat `reopenForumTopic` tepat pada transisi itu. |
...
| FR-TOPIC-08 | P0 | Housekeeping: batas **5 sesi aktif** bersamaan — melebihi itu tawarkan menutup yang terlama; `/pin` mengecualikan dari auto-hapus. Penghapusan otomatis topic `done` setelah 7 hari **dibatalkan**: satu-satunya method untuknya menghapus topic beserta seluruh pesannya, dan yang dibangun adalah penutupan (FR-TOPIC-05). |
