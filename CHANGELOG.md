...
- The pre-release checklist in `docs/security.md` §13 answered against the code as it stands rather than against the audit that filled it in when the version still read 0.2. Ten of its thirteen rows are met, and each names the test that fails when the claim stops being true. Three are deferred with the date and with what has to happen to close them: the incoming-text parsers have no fuzz corpus, `npm audit` runs on no schedule and never sees the optional Baileys peer, and the shipped-defaults claim has no policy-mode gate on the run path to rest on.
